...
class GbParse:
    ...
    def post_parse(self, url, soup):
        post_data = {
            ...
            'url': url,
            'image': soup.fing('img').get('scr'),
            'post_date': datetime.datetime.strptime(soup.find('time', attrs={"itemprop": "datePublished"}).get("datetime"),
                                         "%Y-%m-%dT%H:%M:%S%z"),
        }
    ...
    def comments_parse(self, post_id):
        params = {
            'commentable-id' : post_id,
            'commentable-type': 'Post'
        }
        comments = []
        url_comments = "https://geekbrains.ru/api/v2/comments"
        response = self._get_response(url_comments)
        data = response.json()
        if data:
            for comment in data:
                info = self._get_details
                comments.append.info
        return comments

    def _get_details(self, comment):
        comment_id = comment['id']
        user_author = comment['user']['full_name']
        text = comment ['body']
        comment_details = [{
